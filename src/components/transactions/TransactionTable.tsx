...
export function TransactionTable() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-sm border border-white/[0.08] bg-[var(--origin-surface)] overflow-hidden min-h-[400px] hover:border-white/[0.12]">
        <div className="max-md:hidden grid grid-cols-12 gap-4 p-4 border-b border-white/5 font-mono text-[10px] uppercase tracking-widest text-white/50">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            <div className="loader-pulse-bar w-32 mb-4" />
            <p className="font-mono text-xs text-white/50">Loading history...</p>
          </div>
        ) : !accountId ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-14 h-14 bg-white/5 rounded-sm border border-white/10 flex items-center justify-center mb-4">
              <span className="text-2xl">🔗</span>
            </div>
            <h3 className="text-lg font-medium text-white mb-1">Connect your wallet</h3>
            <p className="font-mono text-xs text-white/50 max-w-xs mx-auto">
      ...
      {!loading && accountId && filteredTransactions.length > 0 && (
        <div className="flex justify-between items-center font-mono text-xs text-white/50 px-2">
